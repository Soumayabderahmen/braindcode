...
const filteredinvestisseurs = computed(() => {
  return props.startupes.filter((r) => {
    const queryMatch =
      !search.value ||
      r.domain_name?.toLowerCase().includes(search.value.toLowerCase()) ||
...
">
          ...
          <option value="">
            ...
            🕒
            Statut</option>
          <option v-for="r in [...new Set(props.startupes.map(res => res.statut))]" :key="r" :value="r">
            {{ r }}
          </option>
