            ...
            <tr v-if="paginatedinvestisseurs.length === 0" class="hover:bg-gray-50">
              <td colspan="6" class="px-6 py-10 text-sm text-center text-gray-500">
                <div class="flex flex-col items-center justify-center">
                  ...
                  <span class="text-lg font-medium">Aucun investisseur trouvé</span>
                </div>
              </td>
            </tr>
            <tr v-for="investisseur in paginatedinvestisseurs" :key="investisseur.id" class="hover:bg-gray-50">
              <td><span>
                  <center>{{ investisseur.name }}</center>
                </span></td>
              <td><span>
                  <center>{{ investisseur.email }}</center>
                </span></td>
              <td><span>
                  <center> {{ investisseur.visibility === 'public' ? '🔓 Public' : '🔐 Private' }}
                  </center>
                </span>
              </td>
              <td style="text-align: center;">
                <span :class="[
                  ...
                  investisseur.statut === 'active'
                    ...
                    : investisseur.statut === 'inactive'
                      ? 'bg-red-100 text-red-800'

                      : 'bg-yellow-100 text-yellow-800'
                ]">

                  <center> {{ investisseur.statut }}</center>
                </span>
              ...
              <td>
                <center>
                  <button v-if="investisseur.statut !== 'active'" @click="activateInvestisseur(investisseur.id)"
